...
3.  Chờ quá trình chạy hoàn tất. Script sẽ tự động cài đặt thư viện cần thiết và dùng PyInstaller để đóng gói ứng dụng.
...
*   **Hiện tượng**: Báo lỗi không tìm thấy file với đường dẫn rất dài khi chạy `build_windows.bat`.
*   **Khắc phục**: Bạn cần bật tính năng "Long Paths" của Windows.
    1.  Nhấn tổ hợp phím `Windows + R`, gõ `regedit` và nhấn Enter.
    2.  Tìm đến đường dẫn: `HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Control\FileSystem`
    3.  Tìm khóa `LongPathsEnabled`, nhấn đúp và sửa giá trị thành `1`.
