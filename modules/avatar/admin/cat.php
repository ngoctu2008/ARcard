...
function recursive_cat_list($parentid, $xtpl, $groups_list, $module_upload, $module_name, $client_info) {
    ...
    while ($row = $result->fetch()) {
        ...
        // Image
        if (!empty($row['image']) and is_file(NV_UPLOADS_REAL_DIR . '/' . $module_upload . '/' . $module_data . '/' . $row['image'])) {
            $row['image'] = NV_BASE_SITEURL . NV_UPLOADS_DIR . '/' . $module_upload . '/' . $module_data . '/' . $row['image'];
        } else {
             $row['image'] = '';
        }
...
while ($cat_item = $result->fetch()) {
     ...
     if (!empty($cat_item['image']) and is_file(NV_UPLOADS_REAL_DIR . '/' . $module_upload . '/' . $module_data . '/' . $cat_item['image'])) {
         $cat_item['image'] = NV_BASE_SITEURL . NV_UPLOADS_DIR . '/' . $module_upload . '/'. $module_data . '/' . $cat_item['image'];
     } else {
         $cat_item['image'] = '';
     }
...
// Layout
$layouts = array('view_grid' => $lang_module['grid'], 'view_list' => $lang_module['list']);
