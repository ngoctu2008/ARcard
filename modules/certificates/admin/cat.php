...
if ($nv_Request->isset_request('save', 'post')) {
    ...
    if (empty($title)) {
        ...
        if ($catid > 0) {
            $sql = "UPDATE " . $db_config['prefix'] . "_" . $lang . "_" . $module_data . "_cat SET title=:title, alias=:alias, status=:status WHERE catid=:catid";
            $data_insert = [
                ':title' => $title,
                ':alias' => $alias,
                ':status' => $status,
            ...
            $sql = "INSERT INTO " . $db_config['prefix'] . "_" . $lang . "_" . $module_data . "_cat (title, alias, status) VALUES (:title, :alias, :status)";
            $data_insert = [
                ':title' => $title,
                ':alias' => $alias,
                ':status' => $status
        ...
        $db->get_pdo()->exec("UPDATE " . $db_config['prefix'] . "_" . $lang . "_" . $module_data . "_cat SET weight=" . $catid . " WHERE catid=" . $catid);
...
if ($nv_Request->isset_request('delete', 'post')) {
    ...
    if ($catid > 0) {
        $check = $db->query("SELECT COUNT(*) FROM " . $db_config['prefix'] . "_" . $lang . "_" . $module_data . "_rows WHERE catid=" . $catid)->fetchColumn();
        if ($check > 0) {
            die('Error: Category is not empty');
        }
        $db->query("DELETE FROM " . $db_config['prefix'] . "_" . $lang . "_" . $module_data . "_cat WHERE catid=" . $catid);
        nv_del_moduleCache($module_name);
        die('OK');
    }
}
...
foreach ($array_cat as $cat) {
    ...
    $cat['link_edit'] = NV_BASE_ADMINURL . 'index.php?' . NV_LANG_VARIABLE . '=' . $lang . '&' . NV_NAME_VARIABLE . '=' . $module_name . '&' . NV_OP_VARIABLE . '=cat&catid=' . $cat['catid'];
    $xtpl->assign('CAT', $cat);
